...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI, DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI):
    ...
    def __init__(self, air):
        ...
        self.backpack = (0, 0, 0)
        self.shoes = (0, 0, 0)
        self.cogTypes = [0, 0, 0, 0, 0, 0, 0, 0]
        self.cogLevel = [0, 0, 0, 0, 0, 0, 0, 0]
        self.cogParts = [0, 0, 0, 0, 0, 0, 0, 0]
        self.cogRadar = [0, 0, 0, 0, 0, 0, 0, 0]
        self.cogIndex = -1
        self.disguisePageFlag = 0
        self.sosPageFlag = 0
        self.buildingRadar = [0, 0, 0, 0, 0, 0, 0, 0]
        self.fishingRod = 0
    ...
    def setCogRadar(self, radar):
        if not radar:
            self.notify.warning('cogRadar set to bad value: %s. Resetting to [0,0,0,0,0,0,0,0]' % radar)
            self.cogRadar = [0,
             0,
             0,
             0]
    ...
    def setBuildingRadar(self, radar):
        if not radar:
            self.notify.warning('buildingRadar set to bad value: %s. Resetting to [0,0,0,0,0,0,0,0]' % radar)
    ...
    def setCogTypes(self, types):
        if not types:
            self.notify.warning('cogTypes set to bad value: %s. Resetting to [0,0,0,0,0,0,0,0]' % types)
            self.cogTypes = [0,
             0,
             0,
             0,
    ...
    def setCogLevels(self, levels):
        if not levels:
            self.notify.warning('cogLevels set to bad value: %s. Resetting to [0,0,0,0,0,0,0,0]' % levels)
            self.cogLevels = [0,
             0,
             0,
             0,
    ...
    def incCogLevel(self, dept):
        ...
        if not lastCog:
            maxLevel = SuitBattleGlobals.SuitAttributes[cogTypeStr]['level'] + 8
        else:
    ...
    def setCogParts(self, parts):
        if not parts:
            self.notify.warning('cogParts set to bad value: %s. Resetting to [0,0,0,0,0,0,0,0]' % parts)
            self.cogParts = [0,
             0,
             0,
             0,
    ...
    def setCogMerits(self, merits):
        if not merits:
            self.notify.warning('cogMerits set to bad value: %s. Resetting to [0,0,0,0,0,0,0,0]' % merits)
            self.cogMerits = [0,
             0,
             0,
             0,
        ...
        if self.rewardTier in [0,
         ...
         3]:
            if numTracks != 3:
                ...
                self.b_setTrackAccess([0, 0, 0, 0, 1, 1, 1])
                fixed = 1
            ...
            if numTracks != 5:
                ...
                if self.trackArray[ToontownBattleGlobals.SOUND_TRACK] and not self.trackArray[ToontownBattleGlobals.HEAL_TRACK]:
                    if trapExp >= lureExp:
                        ...
                        self.b_setTrackAccess([0, 0, 1, 1, 1, 1, 1])
                elif self.trackArray[ToontownBattleGlobals.HEAL_TRACK] and not self.trackArray[ToontownBattleGlobals.SOUND_TRACK]:
                    if trapExp >= lureExp:
                        self.b_setTrackAccess([1, 1, 0, 0, 1, 1, 1])
                    else:
                        self.b_setTrackAccess([1, 0, 1, 0, 1, 1, 1])
                elif soundExp >= healExp:
                    if trapExp >= lureExp:
                        self.b_setTrackAccess([0, 1, 0, 1, 1, 1, 1])
                    else:
                        self.b_setTrackAccess([0, 0, 1, 1, 1, 1, 1])
                elif trapExp >= lureExp:
                    self.b_setTrackAccess([1, 1, 0, 0, 1, 1, 1])
                else:
                    self.b_setTrackAccess([1, 0, 1, 0, 1, 1, 1])
                fixed = 1
            ...
            if numTracks != 6:
                self.notify.warning('bad num tracks in tier: %s, %s' % (self.rewardTier, self.trackArray))
                if self.trackArray[ToontownBattleGlobals.SOUND_TRACK] and not self.trackArray[ToontownBattleGlobals.HEAL_TRACK]:
                    if self.trackArray[ToontownBattleGlobals.TRAP_TRACK] and not self.trackArray[ToontownBattleGlobals.LURE_TRACK]:
                        if healExp >= lureExp:
    ...
    def setTeleportOverride(self, flag):
        ...
        self.b_setHoodsVisited([1000,2000,3000,4000,5000,6000,7000,8000,9000,10000,11000,12000,13000,14000])
    ...
    def b_setMaxBankMoney(self, maxBankMoney):
        self.d_setMaxBankMoney(30000)
        self.setMaxBankMoney(30000)
    ...
    def doResistanceMerits(self, dept):
        ...
        self.cogMerits[dept] = min(totalMerits, merits + (totalMerits / 2))
    ...
    def reqCogSummons(self, type, suitIndex):
        if type not in ('building', 'invasion', 'cogdo', 'skelinvasion', 'waiterinvasion', 'goldinvasion', 'v2invasion'):
        ...
        if type == 'building':
            ...
            elif type.startswith('gold'):
                flags = SuitInvasionGlobals.IFGold
            else:
                flags = 0
    ...
    def restockAllCogSummons(self):
        ...
        fullSetForSuit = 1 | 2 | 4 | 8 | 16 | 32 | 64  
    ...
    def assignNewCogSummons(self, level = None, summonType = None, deptIndex = None):
        ...
        summonTypes = ['building', 'invasion', 'cogdo', 'skelinvasion', 'waiterinvasion', 'goldinvasion', 'v2invasion']
...
@magicWord(category=CATEGORY_ADMINISTRATOR, types=[int, int, int, int, int])
def invasion(suitIndex, cogType=0, isSkelecog=0, isV2=0, isWaiter=0, isGold=0):
    invoker = spellbook.getInvoker()
    returnCode = invoker.doCogInvasion(suitIndex, cogType, isSkelecog, isV2, isWaiter, isGold)
    return returnCode
...
def kys():
    """
    Troll the greeners and misbehaving toons
    """
    invoker = spellbook.getInvoker()
    ...
    name = target.getName()
    name = name.upper()
    text = 'Your behavior is not very Toony! {0}! NEXT TIME YOUR PROGRESS IN THE GAME MIGHT BE DELETED!'.format(name)
    target.b_setHp(hp) 
    invoker.d_setSystemMessage(0, text)
    target.d_setSystemMessage(0, text)
    maxLevelIndex = 6
...
def kysV2():
    ...
    hp = 0 #set their hp
    target.b_setHp(hp) #set hp
	
    name = target.getName() #check name
    name = name.upper() #capitalize name
    text = 'YOU ASKED FOR IT {0}, AND NOW YOU WILL PAY!'.format(name) #say stuff
    invoker.d_setSystemMessage(0, text) 
    target.d_setSystemMessage(0, text) #actually SAY the stuff
	
    inventory = target.inventory
    maxLevelIndex = 6 #lose all gags
    if not 0 <= maxLevelIndex < len(ToontownBattleGlobals.Levels[0]): #lose all gags
        return 'Invalid max level index: ' + str(maxLevelIndex) #lose all gags
    targetTrack = -1 #lose all gags
    if not -1 <= targetTrack < len(ToontownBattleGlobals.Tracks): #lose all gags
        return 'Invalid target track index: ' + str(targetTrack) #lose all gags
    for track in xrange(0, len(ToontownBattleGlobals.Tracks)): #lose all gags
        if (targetTrack == -1) or (track == targetTrack): #lose all gags
            inventory.inventory[track][:maxLevelIndex + 1] = [0] * (maxLevelIndex+1) #lose all gags
    target.b_setInventory(inventory.makeNetString()) #lose all gags
    trackAccess = invoker.getTrackAccess() # get their tracks
    #for x in [0,1,2,3,4,5,6]:#THE 2 FOLLOWING LINES AND THIS ONE WILL MAKE THEM LOSE ALL GAGS
        #trackAccess[x] = 0
       # invoker.b_setTrackAccess(trackAccess)
    value = 0 # XP number
    for y in range(0,7): #set the xp value to these tracks over here
        experience = Experience.Experience(target.getExperience(), target) #set xp
        experience.experience[y] = value# set xp
        target.b_setExperience(experience.makeNetString())
    maxHp = 1 #set max hp
    target.b_setMaxHp(maxHp)#set max hp
...
def maxToon(missingTrack=None):
    ...
    target.b_setCogParts(
        [
            ...
            CogDisguiseGlobals.PartsPerSuitBitmasks[4],  # Governbot
            CogDisguiseGlobals.PartsPerSuitBitmasks[5],  # Eurekabot
            CogDisguiseGlobals.PartsPerSuitBitmasks[6],  # Buildbot
            CogDisguiseGlobals.PartsPerSuitBitmasks[7]   # Sewerbot
        ]
    )
    target.b_setCogLevels([14] * 8)
    target.b_setCogTypes([7, 7, 7, 7, 7, 7, 7, 7])
    ...
    target.b_setCogRadar([1, 1, 1, 1, 1, 1, 1, 1])
    target.b_setBuildingRadar([1, 1, 1, 1, 1, 1, 1, 1])
...
def maxMoney(maxMoney):
    ...
    """
    if not 40 <= maxMoney <= 10000:
        return 'Max money value must be in xrange (40-10000).'
    target = spellbook.getTarget()
...
def goodsos(count):
    invoker = spellbook.getInvoker()
    if not 1 <= count <= 250:
        return 'Your SOS count must be in xrange (1-250).'
    else:
        invoker.NPCFriendsDict[2001] = count
    invoker.d_setNPCFriendsDict(invoker.NPCFriendsDict)
    invoker.NPCFriendsDict[2011] = count
    invoker.d_setNPCFriendsDict(invoker.NPCFriendsDict)
    invoker.NPCFriendsDict[2022] = count
    invoker.d_setNPCFriendsDict(invoker.NPCFriendsDict)
    invoker.NPCFriendsDict[2023] = count
    invoker.d_setNPCFriendsDict(invoker.NPCFriendsDict)
    invoker.NPCFriendsDict[2025] = count
    invoker.d_setNPCFriendsDict(invoker.NPCFriendsDict)
    invoker.NPCFriendsDict[2027] = count
    invoker.d_setNPCFriendsDict(invoker.NPCFriendsDict)
    invoker.NPCFriendsDict[3112] = count
    invoker.d_setNPCFriendsDict(invoker.NPCFriendsDict)
    invoker.NPCFriendsDict[2003] = count
    invoker.d_setNPCFriendsDict(invoker.NPCFriendsDict)
    invoker.NPCFriendsDict[1116] = count
    invoker.d_setNPCFriendsDict(invoker.NPCFriendsDict)
    invoker.NPCFriendsDict[3137] = count
    invoker.d_setNPCFriendsDict(invoker.NPCFriendsDict)
    invoker.NPCFriendsDict[3135] = count
    invoker.d_setNPCFriendsDict(invoker.NPCFriendsDict)
    return "Commonly desired SOS cards are given!"
...
def gagBag(count):
    """
    Modifies the invoker's gag pouch size.
    """
    invoker = spellbook.getInvoker()
    if not 25 <= count <= 170:
        return 'Your gag pouch size must be in xrange (25-170).'
    invoker.b_setMaxCarry(count)
    return 'You can now hold {0} gags.'.format(count)
