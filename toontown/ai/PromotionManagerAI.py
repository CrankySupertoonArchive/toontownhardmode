...
class PromotionManagerAI:
    ...
    def recoverMerits(self, av, cogList, zoneId, multiplier = 1, extraMerits = None, addInvasion = True):
        avId = av.getDoId()
        meritsRecovered = [0, 0, 0, 0, 0, 0, 0, 0]
        if extraMerits is None:
            extraMerits = [0, 0, 0, 0, 0, 0, 0, 0]
        ...
        if meritsRecovered != [0, 0, 0, 0, 0, 0, 0, 0]:
            actualCounted = [0, 0, 0, 0, 0, 0, 0, 0]
            ...
            if reduce(lambda x, y: x + y, actualCounted):
                self.air.writeServerEvent('merits', avId, '%s|%s|%s|%s|%s|%s|%s|%s' % tuple(actualCounted))
