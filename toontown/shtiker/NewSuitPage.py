...
BUILDING_RADAR_POS = ((0.375,
 ...
 -0.225,
 -0.5))
PANEL_COLORS = (Vec4(0.45, 0.5, 0.45, 1),
 Vec4(0.85, 0.88, 0.92, 1),
 Vec4(0.41, 0.78, 0.84, 1),
 Vec4(0.41, 0.91, 0.48, 1))
PANEL_COLORS_COMPLETE1 = (Vec4(0.335, 0.425, 0.23, 1),
 Vec4(0.75, 0.78, 0.8, 1),
 Vec4(0.26, 0.704, 0.683, 1),
 Vec4(0.26, 0.84, 0.253, 1))
PANEL_COLORS_COMPLETE2 = (Vec4(0.375, 0.375, 0.17, 1),
 Vec4(0.98, 0.90, 0.90, 1),
 Vec4(0.46, 0.704, 0.483, 1),
 Vec4(0.46, 0.84, 0.053, 1))
SHADOW_SCALE_POS = ((0.9,
  0,
  10,
  -0.03),
 (0.9,
...
class NewSuitPage(ShtikerPage.ShtikerPage):
    ...
    def load(self):
        ...
        frameModel = loader.loadModel('phase_3.5/models/gui/suitpage_frame2')
        ...
        del frameModel
        self.title = DirectLabel(parent=self.iconNode, relief=None, text=TTLocalizer.NewSuitPageTitle, text_scale=0.1, text_pos=(0.04, 0), textMayChange=0)
        self.radarButtons = []
        icon = icons.find('**/govern_icon')
        self.governRadarButton = DirectButton(parent=self.iconNode, relief=None, state=DGG.DISABLED, image=icon, image_scale=(0.03375, 1, 0.045), image2_color=Vec4(1.0, 1.0, 1.0, 0.75), pos=(-0.2, 10, -0.575), command=self.toggleRadar, extraArgs=[0])
        self.radarButtons.append(self.governRadarButton)
        icon = icons.find('**/eureka_icon')
        self.eurekaRadarButton = DirectButton(parent=self.iconNode, relief=None, state=DGG.DISABLED, image=icon, image_scale=(0.03375, 1, 0.045), image2_color=Vec4(1.0, 1.0, 1.0, 0.75), pos=(-0.2, 10, -0.575), command=self.toggleRadar, extraArgs=[1])
        self.radarButtons.append(self.eurekaRadarButton)
        icon = icons.find('**/build_icon')
        self.buildRadarButton = DirectButton(parent=self.iconNode, relief=None, state=DGG.DISABLED, image=(icon, icon, icon), image_scale=(0.03375, 1, 0.045), image2_color=Vec4(1.0, 1.0, 1.0, 0.75), pos=(-0.2, 10, -0.575), command=self.toggleRadar, extraArgs=[2])
        self.radarButtons.append(self.buildRadarButton)
        icon = icons.find('**/viral_icon')
        self.viralRadarButton = DirectButton(parent=self.iconNode, relief=None, state=DGG.DISABLED, image=(icon, icon, icon), image_scale=(0.03375, 1, 0.045), image2_color=Vec4(1.0, 1.0, 1.0, 0.75), pos=(-0.2, 10, -0.575), command=self.toggleRadar, extraArgs=[3])
        self.radarButtons.append(self.viralRadarButton)
        for radarButton in self.radarButtons:
            radarButton.building = 0
            radarButton.buildingRadarLabel = None
        gui = loader.loadModel('phase_3.5/models/gui/suitpage_gui2')
        self.panelModel = gui.find('**/card')
        self.shadowModels = []
        for index in xrange(1, len(SuitDNA.newSuitHeadTypes) + 1):
    ...
    def unload(self):
        self.ignoreAll()
        self.title.destroy()
        self.governRadarButton.destroy()
        self.eurekaRadarButton.destroy()
        self.buildRadarButton.destroy()
        self.viralRadarButton.destroy()
    ...
    def makePanels(self):
        ...
        xOffset = 0.199
        yOffset = 0.284
        gui = loader.loadModel('phase_3.5/models/gui/suit_detail_panel2')
    ...
    def addQuotaLabel(self, panel):
        ...
        quotaLabel = DirectLabel(parent=panel, pos=(0.0, 0.0, -0.215), relief=None, state=DGG.DISABLED, text=TTLocalizer.NewSuitPageQuota % (count, quota), text_scale=0.045, text_fg=(0, 0, 0, 1), text_font=ToontownGlobals.getSuitFont())
    ...
    def summonButtonPressed(self, panel):
        panelIndex = self.panels.index(panel)
        self.summonDialog = NewSummonCogDialog.NewSummonCogDialog(panelIndex)
    ...
    def addBuildingRadarLabel(self, button):
        ...
        buildingRadarLabel = DirectLabel(parent=button, relief=None, pos=(0.225, 0.0, zPos), state=DGG.DISABLED, image=gui.find('**/avatar_panel'), image_hpr=(0, 0, 90), image_scale=(0.05, 1, 0.1), image_pos=(0, 0, 0.015), text=TTLocalizer.NewSuitPageBuildingRadarP % '0', text_scale=0.05, text_fg=(1, 0, 0, 1), text_font=ToontownGlobals.getSuitFont())
        gui.removeNode()
    ...
    def resetPanel(self, dept, type):
        ...
        panel['text'] = TTLocalizer.NewSuitPageMystery
    ...
    def setPanelStatus(self, panel, status):
        ...
        if status == COG_UNSEEN:
            ...
            suitName = SuitDNA.newSuitHeadTypes[index]
            suitFullName = SuitBattleGlobals.NewSuitAttributes[suitName]['name']
            ...
            if base.localAvatar.cogs[index] < COG_COMPLETE1:
                ...
                quota = str(COG_QUOTAS[1][index % SuitDNA.suitsPerDept])
            panel.quotaLabel['text'] = TTLocalizer.NewSuitPageQuota % (count, quota)
        ...
        if status in (COG_DEFEATED, COG_COMPLETE1, COG_COMPLETE2):
            name = SuitDNA.newSuitHeadTypes[index]
            attributes = SuitBattleGlobals.NewSuitAttributes[name]
            level = attributes['level']
            groupAttacks, singleAttacks = SuitBattleGlobals.getAttacksByType(attributes)
            info = TTLocalizer.NewSuitPageAttackFormat % (level + 1, level + 5, self.getAttackStrings(groupAttacks), self.getAttackStrings(singleAttacks))
    ...
    def getAttackStrings(self, attacks):
        ...
        return string if string else TTLocalizer.NewSuitPageNoAttacks

    def updateAllCogs(self, status):
        for index in xrange(32, len(base.localAvatar.cogs)):
    ...
    def updateCogStatus(self, dept, type, status):
        if dept < 0 or dept > len(SuitDNA.newSuitDepts):
    ...
    def updateCogRadarButtons(self, radars):
        for index in xrange(0, len(radars) / 2):
            if radars[index] == 1:
                self.radarButtons[index]['state'] = DGG.NORMAL
    ...
    def updateCogRadar(self, deptNum, panels, timeout = 0):
        ...
        for panel in panels:
            panel.cogRadarLabel['text'] = TTLocalizer.NewSuitPageCogRadar % panel.count
            if self.radarOn[deptNum]:
                panel.quotaLabel.hide()
                def showLabel(label):
                    label.show()
    ...
    def updateBuildingRadar(self, deptNum, timeout = 0):
        ...
        if button.building:
            ...
            if self.radarOn[deptNum]:
                ...
                if num == 1:
                    button.buildingRadarLabel['text'] = TTLocalizer.NewSuitPageBuildingRadarS % num
                else:
                    button.buildingRadarLabel['text'] = TTLocalizer.NewSuitPageBuildingRadarP % num
