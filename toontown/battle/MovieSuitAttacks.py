...
def doQuake(attack):
    ...
    soundTrack = getSoundTrack('SA_tremor.ogg', delay=1.23, node=suit)
...
def doGavel(attack):
    ...
    gavelTrack = Sequence(getPropAppearTrack(gavel, suit.getRightHand(), posPoints, propDelay, Point3(2.25, 2.25, 2.25), scaleUpTime=0.5))
    gavelTrack.append(Wait(suitDelay))
    hitPoint = toon.getPos(battle)
    hitPoint.setX(hitPoint.getX() + 1.2)
    hitPoint.setY(hitPoint.getY() + 1.5)
    if dmg > 0:
        hitPoint.setZ(hitPoint.getZ() + 1.1)
    movePoint = Point3(hitPoint.getX(), hitPoint.getY() - 1.8, hitPoint.getZ() + 0.2)
    gavelTrack.append(Func(battle.movie.needRestoreRenderProp, gavel))
    gavelTrack.append(Func(gavel.wrtReparentTo, battle))
    gavelTrack.append(getThrowTrack(gavel, hitPoint, duration=throwDuration, parent=battle))
    gavelTrack.append(Wait(0.6))
    gavelTrack.append(LerpPosInterval(gavel, 0.4, movePoint))
    spinTrack = Sequence(Wait(propDelay + suitDelay + 0.2), LerpHprInterval(gavel, throwDuration, Point3(-360, 0, 0)))
    sizeTrack = Sequence(Wait(propDelay + suitDelay + 0.2), LerpScaleInterval(gavel, throwDuration, Point3(6, 6, 6)), Wait(0.95), LerpScaleInterval(gavel, 0.4, MovieUtil.PNT3_NEARZERO))
    propTrack = Sequence(Parallel(gavelTrack, spinTrack, sizeTrack), Func(MovieUtil.removeProp, gavel), Func(battle.movie.clearRenderProp, gavel))
    damageAnims = []
    damageAnims.append(['cringe',
     0.01,
     0.21,
