...
def teleportIn(attack, npc, pos = Point3(0, 0, 0), hpr = Vec3(180.0, 0.0, 0.0)):
    ...
    if npc.getName() == 'Trump':
        f = Func(npc.setChatAbsolute, 'I am team Trump! Fear me %s' % attack['toon'].getName() + ' for I will build the Great Wall of America!', CFSpeech | CFTimeout)
    elif npc.getName() == 'Le Chat Noir':
        f = Func(npc.setChatAbsolute, "Oh damn, why'd you use me %s" % attack['toon'].getName() + "? Congrats, you've just wasted the best SOS card.", CFSpeech | CFTimeout)
    elif npc.getName() == 'Osama Bin Laden':
	    f = Func(npc.setChatAbsolute, 'oh u want my landmines and bomb? k here u go.', CFSpeech | CFTimeout)
    else:
    ...
    if npc.getName() == 'Le Chat Magique':
        magicCatTrack = Sequence()
        magicCatTrack.append(Func(npc.setChatAbsolute, 'GG EZY BOIS', CFSpeech | CFTimeout))
...
def teleportOut(attack, npc):
    ...
    if npc.getName() == 'Trump':
        b = Func(npc.setChatAbsolute, "Aw shoot, oh well I'll just run for president now.", CFSpeech | CFTimeout)
    elif npc.getName() == 'Le Chat Noir':
		b = Func(npc.setChatAbsolute, "Looks like you found the 'hidden' SOS now huh? Yep, that's me.", CFSpeech | CFTimeout)
    elif npc.getName() == 'Osama Bin Laden':
        b = Func(npc.setChatAbsolute, 'Time to kill mericans, shit me have no bomb now.', CFSpeech | CFTimeout)
    else:
