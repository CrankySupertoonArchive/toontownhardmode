...
from toontown.suit.SuitInvasionGlobals import IFSkelecog, IFWaiter, IFGold, IFV2
...
if config.GetBool('want-bossbot-cogdo', False):
    ALLOWED_FO_TRACKS += 'b'
if config.GetBool('want-governbot-cogdo', False):
    ALLOWED_FO_TRACKS += 'v'
if config.GetBool('want-eurekabot-cogdo', False):
    ALLOWED_FO_TRACKS += 'e'
if config.GetBool('want-buildbot-cogdo', False):
    ALLOWED_FO_TRACKS += 'b'
if config.GetBool('want-viralbot-cogdo', False):
    ALLOWED_FO_TRACKS += 'z'
...
class DistributedSuitPlannerAI(DistributedObjectAI.DistributedObjectAI, SuitPlannerBase.SuitPlannerBase):
    ...
    SUIT_BUILDING_TIMEOUT = [
        ...
        192, 180, 168, 156, 144, 132, 120, 108, 96, 84, 72, 60, 48, 36, 24, 12, 6, 3, 1, 0.5
    ]
    ...
    def createNewSuit(self, blockNumbers, streetPoints, toonBlockTakeover=None,
            ...
            suitName=None, skelecog=None, revives=None, waiter=None, gold=None):
        startPoint = None
        blockNumber = None
        if self.notify.getDebug():
    ...
    def upkeepSuitPopulation(self, task):
        ...
        flyInDeficit = ((targetFlyInNum - self.numFlyInSuits) + 4) / 5
        ...
        buildingDeficit = ((targetBuildingNum - self.numBuildingSuits) + 4) / 5
    ...
    def assignSuitBuildings(self, numToAssign):
        ...
        for sp in self.air.suitPlanners.values():
            ...
            numPerTrack['v'] += sp.pendingBuildingTracks.count('v')
            numPerTrack['e'] += sp.pendingBuildingTracks.count('e')
            numPerTrack['b'] += sp.pendingBuildingTracks.count('b')
            numPerTrack['z'] += sp.pendingBuildingTracks.count('z')
        numPerHeight = {
            0: 0,
            1: 0,
            2: 0,
        ...
        while numToAssign > 0:
            ...
            while repeat and (buildingTrack is not None) and (buildingHeight is not None):
                ...
                if numTarget >= SuitBuildingGlobals.buildingMinMax[self.zoneId][1] or numTarget >= numTotalBuildings:
                    ...
                    totalWeightPerTrack[7] -= weight * tracks[7]
    ...
    def pickLevelTypeAndTrack(self, level=None, type=None, track=None):
        def findType(lvl, trk):
            found = False
            possible = SuitBattleGlobals.SuitAttributes.items()
            random.shuffle(possible)
            for key, value in possible:
                if SuitDNA.getSuitDept(key) == trk:
                    testLvl = lvl - value['level'] - 1
                    if testLvl > -1:
                        try:
                            value['hp'][testLvl]
                            found = value['level'] + 1
                            ...
                            continue
            return found
        if type and track and not level:
        ...
        if track is None:
            trkChoices = self.SuitHoodInfo[self.hoodInfoIdx][self.SUIT_HOOD_INFO_TRACK]
            choice = SuitBattleGlobals.pickFromFreqList(trkChoices)
            track = SuitDNA.suitDepts[choice]
        if level is None:
            lvlChoices = self.SuitHoodInfo[self.hoodInfoIdx][self.SUIT_HOOD_INFO_LVL]
            level = random.choice(lvlChoices)
        if type is None:
            i = 0
            type = findType(level, track)
            while not type:
                level = lvlChoices[i]
                type = findType(level, track)
                i += 1
