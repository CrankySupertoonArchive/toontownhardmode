...
class DistributedCashbotBossAI(DistributedBossCogAI.DistributedBossCogAI, FSM.FSM):
    ...
    def makeGoon(self, side = None):
        ...
        if self.getBattleThreeTime() > 1.0:
            goon.STUN_TIME = 6
            goon.b_setupGoon(velocity=10, hFov=90, attackRadius=25, strength=42, scale=2.2)
        else:
            goon.STUN_TIME = self.progressValue(30, 10)
            goon.b_setupGoon(velocity=self.progressRandomValue(5, 9), hFov=self.progressRandomValue(70, 80), attackRadius=self.progressRandomValue(8, 20), strength=int(self.progressRandomValue(10, 36)), scale=self.progressRandomValue(1.0, 2.0))
        goon.request(side)
        return
    ...
    def waitForNextHelmet(self):
        ...
        if currState == 'BattleThree':
            ...
            taskMgr.remove(taskName)
            delayTime = self.progressValue(30, 20)
