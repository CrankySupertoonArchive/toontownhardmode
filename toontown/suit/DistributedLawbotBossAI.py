...
class DistributedLawbotBossAI(DistributedBossCogAI.DistributedBossCogAI, FSM.FSM):
    ...
    def __doneVictory(self, avIds):
        ...
        preferredSummonType = random.choice(['building', 'invasion', 'cogdo', 'skelinvasion', 'waiterinvasion', 'goldinvasion', 'v2invasion'])
        for toonId in self.involvedToons:
            toon = self.air.doId2do.get(toonId)
            if toon:
    ...
    def giveCogSummonReward(self, toon, prefDeptIndex, prefSummonType):
        ...
        if not toon.hasParticularCogSummons(prefDeptIndex, cogLevel, 'building'):
        ...
        elif not toon.hasParticularCogSummons(prefDeptIndex, cogLevel, 'goldinvasion'):
            summonType = 'goldinvasion'
            ...
            for curDeptIndex in xrange(len(SuitDNA.suitDepts)):
                if not toon.hasParticularCogSummons(curDeptIndex, cogLevel, prefSummonType):
                    ...
                    foundOne = True
                    break
                elif not toon.hasParticularCogSummons(curDeptIndex, cogLevel, 'goldinvasion'):
                    summonType = 'goldinvasion'
                    deptIndex = curDeptIndex
                    foundOne = True
                    break
                ...
                possibleSummonType = ['building', 'invasion', 'cogdo', 'skelinvasion', 'waiterinvasion', 'goldinvasion', 'v2invasion']
                typeWeights = ['single'] * 70 + ['building'] * 27 + ['invasion'] * 3
                if not foundOne:
                    for i in xrange(5):
...
def skipCJ(battle='next'):
    ...
    if battle == 'one':
        ...
        return "Starting the first round..."
    ...
    if battle == 'three':
        ...
        return "Skipping to the last round..."
