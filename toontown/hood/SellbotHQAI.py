...
class SellbotHQAI(CogHQAI.CogHQAI):
    ...
    def createFactoryElevators(self):
        ...
        if simbase.config.GetBool('want-hardfactory', True):
            factoryElevator = DistributedFactoryElevatorExtAI(
                self.air, self.air.factoryMgr, ToontownGlobals.HardSellbotFactoryInt, 3)
            factoryElevator.generateWithRequired(ToontownGlobals.SellbotFactoryExt)
            self.factoryElevators.append(factoryElevator)

        if simbase.config.GetBool('want-unfairfactory', True):
            factoryElevator = DistributedFactoryElevatorExtAI(
                self.air, self.air.factoryMgr, ToontownGlobals.UnfairSellbotFactoryInt, 4)
            factoryElevator.generateWithRequired(ToontownGlobals.SellbotFactoryExt)
            self.factoryElevators.append(factoryElevator)
